...
function displayCart(cart) {
    ...
    // Hide empty cart message and show container
    ...
    cartItemsContainer.innerHTML = items.map(item => `
        <div class="bg-surface-container-lowest p-6 rounded-xl flex flex-col sm:flex-row gap-6 transition-transform hover:scale-[1.01] duration-200" data-product-id="${item.product._id}">
            ...
            <div class="flex-grow flex flex-col justify-between">
                <div>
                    <div class="flex justify-between items-start">
                        <h3 class="text-xl font-bold text-on-surface">${escapeHtml(item.product.name)}</h3>
                        <span class="text-xl font-black text-on-surface">$${(item.price * item.quantity).toFixed(2)}</span>
                    </div>
                    <p class="text-sm text-on-surface-variant mt-1">${escapeHtml(item.product.description?.substring(0, 100) || 'No description')}${item.product.description?.length > 100 ? '...' : ''}</p>
...
async function updateCartCountBadge(count = null) {
    ...
    try {
        ...
        if (result.success && result.data) {
            ...
            if (itemCount === 0) cartBadge.classList.add('hidden');
            else cartBadge.classList.remove('hidden');
        }
    } catch (error) {
...
function showToast(message, type = 'success') {
    ...
    toast.className = `fixed bottom-4 right-4 z-50 px-6 py-3 rounded-lg shadow-lg ${type === 'success' ? 'bg-green-500 text-white' : 'bg-red-500 text-white'} animate-bounce`;
...
async function loadUserGreeting() {
    const authAPI = 'http://localhost:5000/api/auth';
    try {
